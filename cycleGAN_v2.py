...
class Model:
    ...
    def __init__(self,
        ...
        with self.graph.as_default():
            if not self.gen_only:
                ...
                self.saver    = tf.compat.v1.train.Saver(var_list=self.list_gen)
    ...
    def create(self):
        ...
        with self.graph.as_default():
            ...
            self.relative_lr    = tf.compat.v1.placeholder_with_default([1.],[1],name="relative_lr")
            self.relative_lr    = self.relative_lr[0]
            
            self.rel_dis_noise  = tf.compat.v1.placeholder_with_default([1.],[1],name="rel_dis_noise")
            ...
            self.genA           = Res_Gen_v2.ResGen('BtoA',self.a_chan,gen_dim=gen_dim,kernel_size=kernel_size,deconv=self.deconv,verbose=self.verbose)
            self.genB           = Res_Gen_v2.ResGen('AtoB',self.b_chan,gen_dim=gen_dim,kernel_size=kernel_size,deconv=self.deconv,verbose=self.verbose)
            ...
            if self.patchgan == 'Patch34':
                ...
                self.disA       = PatchGAN70_v2.PatchGAN70('A',noise=self.dis_noise)
                self.disB       = PatchGAN70_v2.PatchGAN70('B',noise=self.dis_noise)
            ...
            self.disA_His   = HisDis_v2.HisDis('A',noise=self.dis_noise,keep_prob=1.)
            self.disB_His   = HisDis_v2.HisDis('B',noise=self.dis_noise,keep_prob=1.)
        
            # Create a placeholder for the input data
            self.A           = tf.compat.v1.placeholder(tf.float32,[None, None, None, self.a_chan],name="a")
            self.B           = tf.compat.v1.placeholder(tf.float32,[None, None, None, self.b_chan],name="b")
            ...
            self.lambda_c    = tf.compat.v1.placeholder_with_default([self.lambda_c],[1],name="lambda_c")
            self.lambda_c    = self.lambda_c[0]
            self.lambda_h    = tf.compat.v1.placeholder_with_default([self.lambda_h],[1],name="lambda_h")
        ...
        with self.graph.as_default():
            ...
            optimizer_gen   = tf.compat.v1.train.AdamOptimizer(learning_rate=self.relative_lr*0.0002,beta1=0.5)
            ...
            optimizer_dis = tf.compat.v1.train.AdamOptimizer(learning_rate=self.relative_lr*0.0002,beta1=0.5)
