    ...
    def create(self,X,reuse=True):
        ...
        for i in range(num_layers):
            ...
            if i==0 or i==(num_layers - 1):
                ...
                new_conv = tf.compat.v1.layers.conv2d(new_pad,
                                            filters=filters,
                ...
                # Res layers
                new_conv_0 = tf.compat.v1.layers.conv2d(new_pad,
                                            filters=self.gen_dim[i],
                                            kernel_size=self.kernel_size[i],
                                            strides=(1,1),
                                            kernel_initializer=tf.compat.v1.initializers.random_normal(stddev=0.02),
...
#                new_norm_0 = tf.contrib.layers.instance_norm(new_conv_0,reuse=reuse,scale=False,center=False,scope='gen_'+self.gen_name+'_bnorm0_'+str(i),trainable=False)
                new_mean_0, new_var_0 = tf.nn.moments(new_conv_0,axes=(1,2),keepdims=True)
                ...
                new_conv = tf.compat.v1.layers.conv2d(new_pad_0,
                                            filters=self.gen_dim[i],
                                            kernel_size=self.kernel_size[i],
                                            strides=(1,1),
                                            kernel_initializer=tf.compat.v1.initializers.random_normal(stddev=0.02),
                ...
                if self.deconv == 'transpose':
                    new_conv = tf.compat.v1.layers.conv2d_transpose(new_pad,
                                                          filters=self.gen_dim[i],
                                                          kernel_size=self.kernel_size[i],
                                                          strides=(2,2),
                                                          kernel_initializer=tf.compat.v1.initializers.random_normal(stddev=0.02),
                    ...
                    if i == num_layers-3:
                        new_resize = tf.image.resize(new_pad,[tf.cast(tf.shape(X)[1]/2,dtype=tf.int32),\
                                                                     tf.cast(tf.shape(X)[2]/2,dtype=tf.int32)])
    
                    else:
                        new_resize = tf.image.resize(new_pad,[tf.shape(X)[1],tf.shape(X)[2]])
                    ps = int((self.kernel_size[i]-1)/2) # pad size
...
#                                                           scope='gen_'+self.gen_name+'_bnorm_'+str(i),trainable=False)
                new_mean, new_var = tf.nn.moments(new_conv,axes=(1,2),keepdims=True)
